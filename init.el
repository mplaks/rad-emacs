...
;; mlaks i added in visual line mode
...
(custom-set-faces
  ...
  ;; Your init file should contain only one such instance.
  ;; If there is more than one, they won't work right.
 '(default ((t (:inherit nil :stipple nil :background "SystemWindow" :foreground "SystemWindowText" :inverse-video nil :box nil :strike-through nil :overline nil :underline nil :slant normal :weight normal :height 143 :width normal :foundry "outline" :family "Arial")))))
...
;; setup for aspell as flyspell program

;; (setq-default ispell-program-name "c:/mlaks1/aspell/bin/aspell.exe")
...
(defun get-stuff () 
  (interactive)
  (shell-command '"c:/mlaks/get.ahk"))
 
(defun clean-stuff ()
(interactive)
(insert-file-contents "c:/mlaks/tempfiledata")
(goto-char (point-min))
...
(delete-region beg (point-max)))
 (goto-char 1)
(shell-command "C:/mlaks/raise-emacs.ahk")
(message " hi there user"))

(defun get-stuff-full ()
(interactive)
(progn
 (get-stuff)
(clean-stuff)))
...
(interactive)
(save-buffer)
;;(write-region (point-min) (point-max) "c:/mlaks/tempfiledata")
(shell-command '"c:/mlaks/put.ahk")
...
(add-to-list 'default-frame-alist '(background-color . "#ffd700"))
